...
def set_NN(n_equipment: int):
    ...
    net.set_layer(Connected_layer_batch.Connected_layer(16, 20))
    net.set_layer(Activation_layer.Activation_layer(activation_function.relu, activation_function.relu_d))
    net.set_layer(Connected_layer_batch.Connected_layer(20, n_equipment))
...
def main():
    ...
    for n_equipment in range(9, 10):
        ...
        for idx in range(number_runs):
            ...
            loss_results = net.train(batches_agg_train, batches_states_train, n_equipment, False, True)
            ...
            estimations = mixins.denormalize(net.estimate(agg_val_norm, n_equipment, True), min_agg, max_agg)
            new_estimations_zeros = mixins.post_estimation_zeros(estimations, sts_val, n_equipment)

            if idx == 0 and not os.path.exists(f"../../results/deep_learning/HIPE/1_week/fourier/estimated_active_power_{n_equipment}.csv"):
                print(mixins.calculate_error(new_estimations_zeros, eq_val, n_equipment))
                net_best = net
                # save_csv.save_csv(f"../../results/deep_learning/HIPE/1_week/fourier/estimated_active_power_{n_equipment}.csv", agg_val_denorm, new_estimations_zeros, timestamp)
            else:
                # saved_estimations = read_estimations_csv.read_estimations_csv(f"../../results/deep_learning/HIPE/1_week/fourier/estimated_active_power_{n_equipment}.csv")
    ...
    save_csv.save_csv(f"../../results/deep_learning/HIPE/1_week/fourier/estimated_active_power.csv", agg_val_denorm, estimations, timestamp)
