...
def set_NN():
    ...
    net.set_layer(Connected_layer(11, 11))
    net.set_layer(Activation_layer(tanh, tanh_d))
    net.set_layer(Connected_layer(11, 10))
    net.set_layer(Activation_layer(tanh, tanh_d))
...
def main():
    ...
    axs[1, 0].plot(x, loss_results[1])
    axs[2, 0].plot(x, loss_results[2])
    axs[3, 0].plot(x, loss_results[3])
    axs[4, 0].plot(x, loss_results[4])
    axs[0, 1].plot(x, loss_results[5])
    axs[1, 1].plot(x, loss_results[6])
    axs[2, 1].plot(x, loss_results[7])
    axs[3, 1].plot(x, loss_results[8])
    axs[4, 1].plot(x, loss_results[9])
    axs[0, 0].set_title('Equipment 0')
    axs[1, 0].set_title('Equipment 1')
    axs[2, 0].set_title('Equipment 2')
    axs[3, 0].set_title('Equipment 3')
    axs[4, 0].set_title('Equipment 4')
    axs[0, 0].set_title('Equipment 5')
    axs[1, 1].set_title('Equipment 6')
    axs[2, 1].set_title('Equipment 7')
    axs[3, 1].set_title('Equipment 8')
    ...
    save_csv('../../results/deep_learning/HIPE/1_week/estimated_active_power.csv', out, timestamp)
    print(calculate_error(out, eq_val))
