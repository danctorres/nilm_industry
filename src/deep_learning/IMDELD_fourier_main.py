...
def set_NN():
    ...
    net.set_layer(Connected_layer_batch.Connected_layer(20, 6))
...
def main():
    ...
    for idx in range(number_runs):
        ...
        if idx == 0 and not os.path.exists(f"../../results/deep_learning/IMDELD/fourier/estimated_active_power_{n_equipment}.csv"):
            print(mixins.calculate_error(new_estimations_zeros, eq_val, n_equipment))
            save_csv.save_csv(f"../../results/deep_learning/IMDELD/fourier/estimated_active_power_{n_equipment}.csv", agg_val_denorm, new_estimations_zeros, timestamp)
        else:
            save_csv.save_csv(f"../../results/deep_learning/IMDELD/fourier/estimated_active_power_{n_equipment}.csv", agg_val_denorm, new_estimations_zeros, timestamp)
            #saved_estimations = read_estimations_csv.read_estimations_csv(f"../../results/deep_learning/IMDELD/fourier/estimated_active_power_{n_equipment}.csv")
