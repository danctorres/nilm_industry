...
void estimation(double *new_coef, int *num_ON, const float agg_sample, Read_State &st_data, int sample_idx, int number_equipment, std::vector<float> min_coef, std::vector<float> max_coef) {
    agg = agg_sample;
...
void save_coeff(const double *new_coef, int *num_ON, const int number_equipment, const double factor_round, auto &agg_vector, auto &st_data) {
    std::ifstream file("../../../../results/optimization/IMDELD/estimated_coef_" + std::to_string(number_equipment) + ".csv");
    if (file.good()) {
        ...
        std::unique_ptr<Read_Coef> est_coef = std::make_unique<Read_Coef> ("../../../../results/optimization/IMDELD/estimated_coef_" + std::to_string(number_equipment) + ".csv");
        ...
        for (int i = 0; i < agg_vector.size(); i++) {
            for (int j = 0; j < number_equipment; j++) {
                eq_est_old += pol_func(agg_vector[i],
                                                st_data->get_one_parameter(j, i),
                                                est_coef->get_coef_eq(j));
                eq_est_new += pol_func(agg_vector[i],
                                                st_data->get_one_parameter(j, i),
                                                new_coefs_vec[j]);
            }
        ...
        if (error_est_new < error_est_old) {
            ...
            save_coef("../../../../results/optimization/IMDELD/estimated_coef_" + std::to_string(number_equipment) + ".csv", new_coef, num_ON, number_equipment, factor_round);
        }
    }
    else {
