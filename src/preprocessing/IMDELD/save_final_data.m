function [aggregate_training, equipment_training, on_off_training, aggregate_validation, equipment_validation, on_off_validation] = save_final_data(aggregate, on_off, equipment_active_pow, percentage_train, save)
    ...
    if ((save == true) && (size(on_off, 2) == 8))
        ...
        writetable(on_off_validation, join([erase(file_information.Filename,  join(['\src\preprocessing\IMDELD\', file_name, '.m'])), '\data\processed\IMDELD\data_8_equipment\on_off_validation.csv'], '\'));
        writetable(equipment_training, join([erase(file_information.Filename,  join(['\src\preprocessing\IMDELD\', file_name, '.m'])), '\data\processed\IMDELD\data_8_equipment\equipment_training.csv'], '\'));
        writetable(equipment_validation, join([erase(file_information.Filename,  join(['\src\preprocessing\IMDELD\', file_name, '.m'])), '\data\processed\IMDELD\data_8_equipment\equipment_validation.csv'], '\'));
    elseif ((save == true) && (size(on_off, 2) == 7))
        ...
        writetable(on_off_validation, join([erase(file_information.Filename,  join(['\src\preprocessing\IMDELD\', file_name, '.m'])), '\data\processed\IMDELD\data_6_equipment\on_off_validation.csv'], '\'));
        writetable(equipment_training, join([erase(file_information.Filename,  join(['\src\preprocessing\IMDELD\', file_name, '.m'])), '\data\processed\IMDELD\data_6_equipment\equipment_training.csv'], '\'));
        writetable(equipment_validation, join([erase(file_information.Filename,  join(['\src\preprocessing\IMDELD\', file_name, '.m'])), '\data\processed\IMDELD\data_6_equipment\equipment_validation.csv'], '\'));
    end
end
