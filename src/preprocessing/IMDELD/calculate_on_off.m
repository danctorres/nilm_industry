function [on_off_array] = calculate_on_off(data, group_power_limit, save)
    ...
    if save == true
        ...
        writematrix(on_off_array, join([erase(file_information.Filename,  join(['\src\preprocessing\IMDELD\', file_name, '.m'])), '\data\interim\IMDELD\on_off.csv'], '\'));
        saveas(gcf, join([erase(file_information.Filename, ['\src\preprocessing\IMDELD\', file_name, file_ext]), '\reports\figures\on_off.fig'], '') );
        saveas(gcf, join([erase(file_information.Filename, ['\src\preprocessing\IMDELD\', file_name, file_ext]), '\reports\figures\on_off.png'], '') );
    end
end
